...
for link in links:
    ...
    print new_url
    html_doc = session.get(new_url, headers=headers2)
    html_doc.encoding = 'utf-8'
    soup_doc = BeautifulSoup(html_doc.text, "lxml")
    #print soup_doc.prettify()
    doc = soup_doc.pre
...
print nextpage_url
html = session.get(nextpage_url, headers=headers2)
html.encoding = 'utf-8'
soup = BeautifulSoup(html.text, "lxml")
#print soup.prettify()
nextpages =  soup.find_all('a', href=re.compile('^FJUDQRY02'))
nextpage_url = url_base+nextpages[2]['href']
print len(nextpages)
while len(nextpages) != 4:
    links =  soup.find_all('a', href=re.compile('^FJUDQRY03'))
    for link in links:
        new_url = url_base+link['href']
        print new_url
        html_doc = session.get(new_url, headers=headers2)
        html_doc.encoding = 'utf-8'
        soup_doc = BeautifulSoup(html.text, "lxml")
        #print soup_doc.prettify()
        doc = soup_doc.pre
        print doc
        time.sleep( 1 )
